...
async def processar_motivo_rejeicao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if motivo == "invalido":
        ...
        caption_para_admin = f"⚠️ *AÇÃO NECESSÁRIA*\n\nO comprovante do usuário foi marcado como *Inválido* e ele foi instruído a enviar um novo\\. Aguardando nova submissão\\."
        await query.edit_message_caption(caption=caption_para_admin, parse_mode=ParseMode.MARKDOWN_V2)
...
# --- FUNÇÕES DE ADMINISTRAÇÃO E MANUTENÇÃO (RESTAURADAS) ---
...
async def enviar_lembretes_de_renovacao_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    lembretes_a_enviar = {
        7: "(lembrete_enviado_dias IS NULL OR lembrete_enviado_dias > 7)",
        3: "(lembrete_enviado_dias IS NULL OR lembrete_enviado_dias > 3)",
        1: "(lembrete_enviado_dias IS NULL OR lembrete_enviado_dias > 1)"
    }
...
def configure_application():
    ...
    application.add_handler(CallbackQueryHandler(iniciar_fluxo_renovacao, pattern="^renovar_"))
...
async def pre_run_bot_operations(application: Application):
    ...
    admin_commands = user_commands + [
        ...
        BotCommand("dashboard", "📈 Ver resumo do negócio")
    ]
